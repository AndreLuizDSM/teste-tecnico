...
```bash
git clone https://github.com/AndreLuizDSM/teste-tecnico
cd teste-tecnico
```
...
O PostgreSQL já sobe configurado e pronto na porta `5432`.
...
O projeto já possui valores default configurados no `application.properties` —
não é necessário criar o `.env` para rodar localmente.

Se quiser customizar, crie um `.env` com base no `.env.example`:
...
> Você pode gerar com: `echo -n "sua-senha-longa" | base64`
...
### PostgreSQL
Optei por PostgreSQL mesmo em desenvolvimento para garantir paridade com o ambiente de produção. O trade-off é a necessidade de ter o banco instalado localmente se não decidir usar o docker.
...
- Docker file do back-end e front-end para rodar o projeto com um comando.
